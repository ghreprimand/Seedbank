...
const ORGANIC_MODES: Array<{ label: string; prompt: string }> = [
  {
    label: 'What if?',
    prompt: 'Give one compact thought for this idea in this exact shape: one grounded insight, one actionable next move, and one provocative "what if" question. Ground it in at least two concrete details from this idea\'s raw notes, concept, build notes, risks, or validation criteria. Use future-facing language unless the context explicitly says the project already exists or has been dogfooded. If this is a personal daily-driver or learning project, frame it around the workflow the user intends to improve, not completed usage data. Keep it to 3 short sentences. Do not include headings, labels, bullets, or markdown.',
  },
  {
    label: "Devil's Advocate",
    prompt: 'Give one compact challenge for this idea in this exact shape: one grounded concern, one actionable way to test or reduce that concern before/during the first build, and one follow-up question. Challenge only an assumption that is actually present in this idea context, and work the assumption and evidence into natural sentences instead of labels. Use future-facing language unless the context explicitly says the project already exists or has been dogfooded. If this is a personal daily-driver or learning project, test it against the workflow the user intends to improve rather than launch or external-user metrics unless external users are explicitly mentioned. If the context is too sparse to identify one, ask for the missing detail instead of inventing a critique. Keep it to 3 short sentences. Do not include headings, labels, bullets, or markdown.',
  },
  {
    label: 'Scope Down',
    prompt: 'Give one compact scope-down thought for this idea in this exact shape: one grounded insight about the core value, one actionable first-build scope cut, and one question that removes scope without removing that value. Anchor on the stated personal value, differentiators, or phase plan. Use future-facing language unless the context explicitly says the project already exists. Keep it to 3 short sentences. Do not include headings, labels, bullets, or markdown.',
  },
  {
    label: 'User Story',
    prompt: 'Give one compact first-use thought for this idea in this exact shape: one concrete first-use or first-dogfood scenario, one actionable thing to prepare or observe in that scenario, and one follow-up question. If the notes frame this as a personal daily-driver, make the use case about the user\'s intended repeated workflow; only invent an external user when the notes imply one. Use future-facing language unless the context explicitly says the project already exists. Keep it to 3 short sentences. Do not include headings, labels, bullets, or markdown.',
  },
];
