    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'ul', items: [
            ...
            'OpenAI-compatible endpoint — configure any service that accepts OpenAI Chat Completions format: OpenRouter, Groq, Mistral, LM Studio, vLLM, llama.cpp, LocalAI, or a custom gateway.',
          ]},
          { type: 'h3', text: 'Provider API keys vs. Seedbank tokens' },
          { type: 'p', text: 'Provider API keys (OpenAI, Anthropic) are stored server-side, encrypted at rest. The browser never sees the raw value — only a "has key" boolean. These are separate from Seedbank personal access tokens, which are bearer tokens for the Seedbank REST API itself (Settings → API & Server).' },
          ...
          { type: 'ul', items: [
            ...
            'Cloud providers (OpenAI, Anthropic, OpenRouter, Groq, Mistral, Together, Fireworks) — field content from your ideas is sent to that provider\'s servers when AI features run.',
            'Custom endpoint — data residency depends on where your endpoint runs. Verify with your endpoint\'s operator.',
          ]},
          { type: 'tip', text: 'To keep all AI inference private and local, configure Ollama or a local OpenAI-compatible endpoint (LM Studio, vLLM, or llama.cpp) and set it as the global default.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'Add local/network folder destinations or rclone remotes. Each destination can be tested before use and can independently include DB and/or JSON artifacts.' },
          ...
          { type: 'p', text: 'Run restore validation to verify backup files without replacing live data. Validation is limited to Seedbank backup/export directories and configured local backup destinations.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'ul', items: [
            'Local project scaffold (built-in) — creates a standalone project directory with idea context files. Works with any local development workflow.',
            'Custom local adapters (optional) — optional adapters can integrate with a specific local workflow tool. Configured via Settings → Integrations. Not required to use Seedbank.',
          ]},
    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'When an idea is ready to ship, graduation adapters create a project scaffold in an external project root you configure. The built-in local project scaffold adapter works with any directory and any toolchain. Optional custom adapters integrate with specific local tools — they are not required.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'Optional adapters can integrate with a specific local workflow tool or project manager. Each adapter needs a workspace root and a project root configured in Settings → Integrations. Custom adapters are not required and do not affect any other Seedbank features.' },
