    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'Choose Custom / OpenAI-compatible endpoint and select your server from the "Local servers" group in the preset selector. Leave the API key blank — local servers do not require one. Confirm the base URL matches the port your server uses. Idea content stays on this machine.' },
          { type: 'h3', text: 'I use OpenRouter, Groq, Mistral, Together, or Fireworks' },
          { type: 'p', text: 'Choose Custom / OpenAI-compatible endpoint and select your service from the "Cloud / external services" group in the preset selector. Enter your API key for that service. Idea content is sent to the service\'s servers. Use the preset selector to populate the base URL quickly, then enter the model ID.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'Provider cards are organised in four groups. The selected global default is used by all AI features unless overridden in Feature Defaults (see below).' },
          { type: 'h3', text: 'Direct API providers' },
          { type: 'ul', items: [
          ...
          { type: 'ul', items: [
            'Ollama — set the base URL (default: http://localhost:11434). No API key required. Idea content stays on the configured Ollama host.',
          ]},
          { type: 'h3', text: 'External & custom endpoints' },
          { type: 'ul', items: [
            'Custom / OpenAI-compatible endpoint — configure any service that accepts OpenAI Chat Completions requests. The preset selector groups options: local servers (LM Studio, vLLM, llama.cpp, LocalAI — content stays on this machine) and cloud/external services (OpenRouter, Groq, Mistral, Together, Fireworks — content is sent to their servers).',
          ]},
          { type: 'h3', text: 'Account & subscription transports' },
