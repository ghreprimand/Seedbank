    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'By default the Thinking Partner uses the global AI provider set in Settings → AI & Agents. You can route it to a different provider in Feature Defaults → Thinking Partner. Provider families: direct API-key (OpenAI API, Anthropic API), local inference (Ollama, LM Studio, vLLM, llama.cpp, LocalAI), external cloud endpoints (OpenRouter, Groq, Mistral, Together, Fireworks), and account/subscription transports (Claude account — coming soon; Codex account — experimental).' },
...

  {
    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          { type: 'p', text: 'Not sure which AI method to pick? Use this guide to match your setup. In Settings → AI & Agents, methods are grouped by service family (Claude, Codex/OpenAI, local inference, cloud routers).' },
          { type: 'h3', text: 'I have a Claude subscription (claude.ai)' },
          { type: 'p', text: 'In the Claude service area, the Claude account method includes login and status controls but remains coming-soon in this RC. For stable Claude routing now, use the Anthropic API key method. Visit console.anthropic.com to create a key.' },
          ...
          { type: 'p', text: 'In the Codex/OpenAI service area, the Codex account method is experimental and requires the separate Codex component plus server opt-in (`SEEDBANK_ENABLE_CODEX_ACCOUNT`). If you want stable OpenAI model routing without that path, use the OpenAI API key method from platform.openai.com.' },
          ...
          { type: 'p', text: 'Use the Local inference area and choose the Local OpenAI-compatible method. Select LM Studio, vLLM, llama.cpp, LocalAI, or custom localhost URL. Local servers usually do not need an API key. Confirm the base URL/port and model ID.' },
          { type: 'h3', text: 'I use OpenRouter, Groq, Mistral, Together, or Fireworks' },
          { type: 'p', text: 'Use the External/cloud routers area and choose the cloud OpenAI-compatible method. Select OpenRouter, Groq, Mistral, Together, Fireworks, or custom cloud URL. Enter that service\'s API key and model ID.' },
          { type: 'tip', text: 'To keep all AI inference private on this machine, use Ollama or a local custom endpoint (LM Studio, vLLM, or llama.cpp) with a localhost URL, and enable Local-only mode in Usage & Guardrails. Note: Feature Defaults can route individual features to different providers, so also check those settings.' },
        ],
      },
      {
        id: 'settings-ai',
        ...
        blocks: [
          { type: 'p', text: 'Settings → AI & Agents is where you configure service methods, per-feature routing, token budgets, and guardrails.' },
          { type: 'h3', text: 'Service-first layout' },
          { type: 'p', text: 'Configuration is grouped by service family first, then method: Claude service, Codex/OpenAI service, Local inference, and External/cloud routers.' },
          { type: 'h3', text: 'Claude service' },
          { type: 'ul', items: [
            'Anthropic API method — API-key chat/model routing for Claude models.',
            'Claude account method — account-auth method with login/status controls; still coming-soon posture for RC routing.',
            'Claude Code CLI method — file-producing agent method; not used in Feature Defaults chat routing.',
          ]},
          { type: 'h3', text: 'Codex/OpenAI service' },
          { type: 'ul', items: [
            'OpenAI API method — API-key chat/model routing for OpenAI models.',
            'Codex account method (experimental) — account-auth routing path behind `SEEDBANK_ENABLE_CODEX_ACCOUNT`.',
            'Codex CLI method — file-producing agent method; not used in Feature Defaults chat routing.',
          ]},
          { type: 'h3', text: 'Local inference' },
          { type: 'ul', items: [
            'Ollama — set base URL and model; no API key required.',
            'Local OpenAI-compatible method — LM Studio, vLLM, llama.cpp, LocalAI, and custom localhost URL.',
          ]},
          { type: 'h3', text: 'External/cloud routers' },
          { type: 'ul', items: [
          ...
          { type: 'p', text: 'The "Set default" button on chat/model-capable method cards sets the global default provider. All AI features use this unless overridden in Feature Defaults.' },
          { type: 'h3', text: 'Feature Defaults' },
          { type: 'p', text: 'Feature Defaults routes each named AI feature to a specific provider/model independently of global default. Only chat/model-capable providers are routable; CLI file-agent methods are excluded.' },
          { type: 'ul', items: [
          ...
          { type: 'tip', text: 'The privacy notice reflects the global default provider only. Feature Defaults can route individual features to different providers — check each Feature Default row to confirm data residency for every feature. To keep all AI inference private, set local providers for every Feature Default row and enable Local-only mode.' },
          ...
          { type: 'h3', text: 'CLI agent methods' },
          { type: 'p', text: 'Claude Code and Codex CLI link controls now live inside their respective service areas. They use CLI-managed auth and power file-producing agent runs, not normal chat routing.' },
