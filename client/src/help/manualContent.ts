    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'Claude account login lets you sign in with your Claude.ai account and route AI chat through it using OAuth. The Claude account card is available when the server has SEEDBANK_ENABLE_CLAUDE_ACCOUNT set. Click "Log in with Claude" in the card and follow the browser prompt. Once authenticated, set it as the global default or route individual features to it in Feature Defaults. Idea content is sent to Anthropic\'s servers.' },
          { type: 'p', text: 'If you do not have SEEDBANK_ENABLE_CLAUDE_ACCOUNT set, or prefer a stable API key path, use the Anthropic API provider instead. Visit console.anthropic.com to create a key.' },
          ...
          { type: 'p', text: 'Codex account login routes AI chat through the Codex app-server, which communicates with OpenAI on your behalf. This is experimental and requires the Codex component installed locally plus SEEDBANK_ENABLE_CODEX_ACCOUNT set server-side. If you want stable OpenAI model routing without that path, use the OpenAI API key method from platform.openai.com.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'Providers are grouped by service family: Claude service, Codex/OpenAI service, Local inference, and External/cloud. Within each family, distinct connection modes — API key or Account login — are separate configured instances with their own settings.' },
          ...
          { type: 'ul', items: [
            ...
            'Claude account login — OAuth-based routing using your Claude.ai subscription (requires SEEDBANK_ENABLE_CLAUDE_ACCOUNT). Login and logout from the card; token refresh is automatic.',
          ]},
          ...
          { type: 'ul', items: [
            ...
            'Codex account login (experimental) — routes through local Codex app-server via JSON-RPC. Requires SEEDBANK_ENABLE_CODEX_ACCOUNT and a compatible Codex install. Effort selector for reasoning models.',
          ]},
          { type: 'tip', text: 'Claude Code CLI and Codex CLI are file-producing agents, not AI chat providers. They are configured in the Agents area below the provider cards and are not available in Feature Defaults.' },
          ...
          { type: 'ul', items: [
            ...
            'Local OpenAI-compatible — independent config for LM Studio, vLLM, llama.cpp, LocalAI, or a custom localhost URL. Separate from Cloud OpenAI-compatible — each has its own URL, key, and model.',
          ]},
          { type: 'h3', text: 'External / cloud' },
          { type: 'ul', items: [
            'Cloud OpenAI-compatible — independent config for OpenRouter, Groq, Mistral, Together, Fireworks, or a custom HTTPS endpoint. Separate from Local OpenAI-compatible.',
          ]},
          ...
          { type: 'p', text: 'The star (★) button on any configured provider instance marks it as the global default. All AI features inherit this unless overridden in Feature Defaults.' },
          { type: 'h3', text: 'Feature Defaults' },
          { type: 'p', text: 'Routes each named AI feature to a specific provider, model, and effort level independently. Only API-key and account-login providers are available here; CLI file-agent tools are excluded.' },
          { type: 'ul', items: [
          ...
          { type: 'p', text: 'Setting a feature\'s provider to "Use global default" means it inherits whichever instance is currently marked with ★. The Effective readout beneath each row shows the resolved provider, model, and effort that will actually be used — accounting for inheritance.' },
          ...
          { type: 'ul', items: [
            ...
            'Provider enable — disable specific provider instances server-side.',
          ...
          { type: 'p', text: 'Claude Code and Codex CLI are configured in their own agent cards below the provider sections. They use CLI-managed credentials, power file-producing agent runs on idea workspaces, and are not part of AI chat routing.' },
    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'h3', text: 'CLI agents' },
          { type: 'p', text: 'Link Claude Code or Codex CLI in Settings → AI & Agents to run guided, review-first development sessions from an idea.' },
