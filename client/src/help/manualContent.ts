    ...
    sections: [
      {
        ...
        blocks: [
          { type: 'p', text: 'Seedbank can launch a local CLI agent (Claude Code or Codex CLI) against a scratch workspace seeded with your idea\'s content. This is separate from the Thinking Partner chat, field suggestions, and all other AI chat features — agents are file-producing tools that work in a workspace, not AI providers for chat or model routing.' },
          { type: 'tip', text: 'CLI agents do not appear in Feature Defaults and cannot be selected as AI providers for Thinking Partner, field suggestions, health check, or discover insights. Those use direct API-key or account-login providers configured in Settings → AI & Agents.' },
    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'Codex account login routes AI chat through the local Codex app-server, which communicates with OpenAI on your behalf. This requires a compatible Codex runtime installed locally. If you want direct OpenAI model routing without that path, use the OpenAI API key method from platform.openai.com.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'ul', items: [
            ...
            'Claude account login — OAuth-based routing using your Claude.ai subscription. Login and logout from the card; token refresh is automatic.',
          ]},
          ...
          { type: 'ul', items: [
            ...
            'Codex account login — routes through local Codex app-server via JSON-RPC. Requires a compatible Codex install. Effort selector for reasoning models.',
          ]},
          { type: 'tip', text: 'Claude Code CLI and Codex CLI are file-producing agents, not AI chat providers. They are linked through the agents API surface and are not available in Feature Defaults.' },
          ...
          { type: 'p', text: 'Claude Code and Codex CLI are linked through the agents API endpoints (not through Feature Defaults). They use CLI-managed credentials, power file-producing agent runs on idea workspaces, and are not part of AI chat routing.' },
          { type: 'tip', text: 'Provider API keys are stored server-side, encrypted. The browser never sees the raw key. CLI agent credentials are never stored in Seedbank at all.' },
        ],
      },
      {
        id: 'settings-theme',
    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'h3', text: 'CLI agents' },
          { type: 'p', text: 'Link Claude Code or Codex CLI through the agents API endpoints to run guided, review-first development sessions from an idea.' },
