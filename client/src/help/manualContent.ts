    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'h3', text: 'Personal access tokens' },
          { type: 'p', text: 'Generate scoped bearer tokens in Settings → API & Server to authenticate API requests. Scopes: read:ideas (read-only), write:ideas (create/update/graduate), ai:suggest (AI endpoints). Tokens are intended for local scripting and integrations — creation is restricted to localhost sessions.' },
          ...
          { type: 'p', text: 'Link a local Claude Code or Codex CLI binary in Settings → AI & Agents. Use "Develop with agent" on any idea to launch a sandboxed run with the idea as context. The agent works in a scratch workspace; you review proposed files before anything is saved.' },
      ...
      },
      {
        ...
        blocks: [
          { type: 'p', text: 'The MCP endpoints make your seed garden readable as context for external AI sessions. They are read-only and require a bearer token with read:ideas scope.' },
          { type: 'ul', items: [
          ...
          { type: 'tip', text: 'Create a dedicated token with read:ideas scope (not write:ideas) for MCP access. Revoke it from Settings → API & Server if it is no longer needed.' },
        ],
      },
      {
