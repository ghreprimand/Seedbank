...
export const MANUAL_GROUPS: ManualGroup[] = [
    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'tip', text: 'All non-deleted ideas are shown by default, including Shelved and Cold Storage. Use Stage filters to focus specific lifecycle states.' },
        ],
      },
      {
        id: 'idea-editing',
        ...
        blocks: [
          ...
          { type: 'h3', text: 'Scores' },
          { type: 'p', text: 'Rate Personal Excitement (🔥) and Jam Suitability (⚡) on a 1–5 scale. Scores are used for sorting and health checks.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'ul', items: [
            ...
            '📦 Shelved — paused but preserved.',
            '❄️ Cold Storage — deep archive, still searchable.',
            '🚀 Shipped — done, released, or completed.',
          ]},
          { type: 'tip', text: 'Change stage any time from the idea detail stage picker. Graduation typically advances to build stages (for example, Prototype), not directly to Shipped.' },
        ],
      },
      {
        id: 'categories',
    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'ul', items: [
            ...
            'Excitement / jam suitability score balance.',
          ]},
      ...
      },
      {
        ...
        blocks: [
          { type: 'p', text: 'Fields like Pitch, Full Notes, Hook, Why It Might Work, Risks, and Tech Stack have a ✨ button that opens the AI Assistance modal. You choose how you want help, review the result, and apply only what you want.' },
...

  {
    ...
    sections: [
      {
        id: 'agents',
        title: 'Optional CLI Agents',
        indexLabel: 'Agents Overview',
        keywords: ['agent', 'claude code', 'codex', 'cli', 'run', 'develop', 'generate', 'scaffold', 'transcript', 'auth', 'environment'],
        blocks: [
          { type: 'p', text: 'Seedbank can optionally launch a local CLI agent (Claude Code or Codex CLI) against a scratch workspace seeded with your idea\'s content. This is separate from Claude native OAuth, Codex app-server auth, Thinking Partner chat, field suggestions, and all other AI chat features — agents are file-producing tools that work in a workspace, not AI providers for chat or model routing.' },
          { type: 'tip', text: 'CLI agents do not appear in Feature Defaults and cannot be selected as AI providers for Thinking Partner, field suggestions, health check, or discover insights. Those use direct API-key providers or account transports configured in Settings → AI & Agents.' },
          { type: 'h3', text: 'Linking an agent' },
          { type: 'p', text: 'Agent linking is configured through API endpoints, not a dedicated settings card. Use POST /api/agents/link with provider (claude or codex) and cliPath; Seedbank runs --version to validate and stores only link metadata.' },
          { type: 'h3', text: 'Agent authentication' },
          { type: 'p', text: 'Agents use their own CLI-managed credentials — not Seedbank\'s provider API keys, Claude account OAuth, or Codex account app-server auth. Seedbank stores only the binary path and a linked flag.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'ul', items: [
            ...
            'Agents inherit the environment variables of the Seedbank server process, so only run/link CLI tools you trust in that environment.',
...

  {
    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          { type: 'p', text: 'Not sure which AI provider to pick? Use this guide to match your setup. Settings → AI & Agents groups providers by connection type: direct API key, account login, local inference, and external/cloud.' },
          { type: 'h3', text: 'I have a Claude subscription (claude.ai)' },
          { type: 'p', text: 'Claude account login lets you sign in with your Claude.ai account and route AI chat through it using Seedbank\'s native OAuth flow. Click "Log in with Claude" in the card and follow the browser prompt. Once authenticated, set it as the global default or route individual features to it in Feature Defaults. Idea content is sent to Anthropic\'s servers.' },
          { type: 'p', text: 'If you prefer a direct API-key path, use the Anthropic API provider instead. Visit console.anthropic.com to create a key.' },
          ...
          { type: 'p', text: 'Codex account login routes AI chat through the local Codex app-server auth flow, which communicates with OpenAI on your behalf. This requires a compatible Codex runtime installed locally. If you want direct OpenAI model routing without that path, use the OpenAI API key method from platform.openai.com.' },
          ...
          { type: 'p', text: 'Ollama runs AI models on your own machine. In the Local inference area, set the Ollama base URL (default: http://localhost:11434), enter the name of a model you have pulled (e.g. llama3.2 or mistral), and set it as the global default. No API key is needed. Idea content stays on your configured Ollama host.' },
          { type: 'h3', text: 'I use a local server like LM Studio, vLLM, llama.cpp, or LocalAI' },
          { type: 'p', text: 'In the Local inference area, choose Local OpenAI-compatible and select LM Studio, vLLM, llama.cpp, LocalAI, or custom localhost URL. Local servers usually do not need an API key. Confirm the base URL/port and model ID. Idea content stays on the configured local host.' },
          { type: 'h3', text: 'I use OpenRouter, Groq, Mistral, Together, or Fireworks' },
          { type: 'p', text: 'In the External/cloud area, choose Cloud OpenAI-compatible and select OpenRouter, Groq, Mistral, Together, Fireworks, or custom cloud URL. Enter that service\'s API key and the model ID you want to use. Idea content is sent to that provider\'s servers.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'Providers are grouped by service family: Claude service, Codex/OpenAI service, Local inference, and External/cloud. Within each family, distinct connection modes — API key, Account login, local server, or external router — are separate configured instances with their own settings.' },
          { type: 'h3', text: 'Claude service' },
          { type: 'ul', items: [
            'Anthropic API — direct API-key routing for Claude models. Enter key from console.anthropic.com; model list auto-fetched.',
            'Claude account login — native OAuth routing using your Claude.ai subscription. Login and logout from the card; token refresh is automatic.',
          ]},
          { type: 'h3', text: 'Codex / OpenAI service' },
          { type: 'ul', items: [
            'OpenAI API — direct API-key routing for OpenAI models. Effort selector shown for reasoning models (o3, o4-mini, etc.).',
            'Codex account login — routes through local Codex app-server auth via JSON-RPC. Requires a compatible Codex install. Effort selector for reasoning models.',
          ]},
          ...
          { type: 'tip', text: 'Claude Code CLI and Codex CLI are optional file-producing agents, not Claude/Codex chat account auth. They are linked through the agents API surface and are not available in Feature Defaults.' },
          ...
          { type: 'ul', items: [
            ...
            'Local OpenAI-compatible — add one or more saved LM Studio, vLLM, llama.cpp, LocalAI, or custom localhost instances. Each instance has its own label, URL, model list, and probe status.',
          ]},
          { type: 'h3', text: 'External / cloud' },
          { type: 'ul', items: [
            'Cloud OpenAI-compatible — add one or more saved OpenRouter, Groq, Mistral, Together, Fireworks, or custom HTTPS instances. For broad catalogs, use the enabled-model checklist to choose which discovered models Seedbank should expose.',
          ]},
          ...
          { type: 'p', text: 'The Set default button on any configured provider instance marks it as the global default. The global default row in Feature Defaults controls the default provider instance, model, and effort where supported.' },
          { type: 'h3', text: 'Feature Defaults' },
          { type: 'p', text: 'Routes each named AI feature to a specific provider instance, model, and effort level independently. Provider/model controls use discovered model dropdowns when available and still allow custom model IDs. CLI file-agent tools are excluded.' },
          { type: 'ul', items: [
            'Thinking Partner — the conversational chat panel on each idea page.',
            'Field suggestions — the "Ask AI" button on Pitch, Full Notes, Hook, Why It Might Work, Risks, and Tech Stack fields.',
          ...
          { type: 'h3', text: 'Optional CLI agents' },
          { type: 'p', text: 'Claude Code and Codex CLI, when used, are linked through the agents API endpoints (not through Feature Defaults). They power file-producing agent runs on idea workspaces and are not part of Claude/Codex chat account auth or AI chat routing.' },
          { type: 'tip', text: 'Provider API keys are stored server-side, encrypted. The browser never sees the raw key. The reauth reminder stores only a local "previously authenticated" flag, not credentials. CLI agent credentials are never stored in Seedbank at all.' },
        ],
      },
      {
        id: 'settings-theme',
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'ul', items: [
            'Tokens are scoped: read:ideas, write:ideas, ai:suggest, mcp:read, and agents:run.',
    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'ul', items: [
            ...
            'CLAUDE.md — starter context for Claude Code, Codex, or another AI coding tool.',
          ...
          { type: 'h3', text: 'Using README.md and CLAUDE.md' },
          { type: 'p', text: 'Open README.md first to confirm goals, scope, and constraints. Then use CLAUDE.md as ready-to-paste context when you start an AI coding session, so the assistant has your project intent from the start.' },
    ...
    sections: [
      {
        ...
        blocks: [
          ...
          { type: 'h3', text: 'Optional CLI agents' },
          { type: 'p', text: 'If you use the separate file-producing agent runner, link Claude Code or Codex CLI through the agents API endpoints to run guided, review-first development sessions from an idea.' },
          { type: 'tip', text: 'Project scaffold setup lives in Settings → Project Graduation and the Graduation help section, not in API & Automation.' },
        ],
      },
      {
