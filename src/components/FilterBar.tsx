...
interface FilterBarProps {
  /** All tags present in the current data set (for the tag picker) */
  availableTags: string[];
  /** Total count of ideas (before filtering) */
  totalCount: number;
...
export default function FilterBar({ availableTags, totalCount, filteredCount }: FilterBarProps) {
  ...
  return (
    <div className="space-y-2">
      {/* Top row: filter buttons + sort + count */}
      <div className="flex flex-wrap items-center gap-2">
        {/* Category dropdown */}
        <div className="relative">
          <button
            onClick={() => toggle('category')}
            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-pill border transition-colors ${
              categories.length > 0
                ? 'bg-sage-100 border-sage-300 text-sage-800'
                : 'bg-paper border-ink-200 text-ink-500 hover:border-ink-300'
            }`}
          >
            Category
            {categories.length > 0 && (
              <span className="ml-0.5 px-1.5 py-0.5 text-[10px] bg-sage-200 rounded-full">
                {categories.length}
              </span>
            )}
        ...
        <div className="relative ml-auto">
          <button
            onClick={() => toggle('sort')}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-pill border border-ink-200 text-ink-500 hover:border-ink-300 bg-paper transition-colors"
          >
            ...
            <DropdownPanel onClose={() => setOpenPanel(null)} align="right">
              {SORT_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`block w-full text-left px-3 py-1.5 text-xs rounded-badge transition-colors ${
                    sortBy === opt.field
      ...
      {hasFilters && (
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <button
            onClick={clearAll}
            className="ml-2 flex items-center gap-1 px-2 py-0.5 text-[11px] text-ink-400 hover:text-ink-600 hover:bg-ink-100 rounded-badge transition-colors"
          >
            <X className="w-3 h-3" />
            Clear all
          </button>
...
function DropdownPanel({
  ...
  return (
    <>
      ...
      <div
        className={`absolute top-full mt-1 z-30 bg-paper border border-ink-200 rounded-card shadow-modal p-2 min-w-[180px] flex flex-wrap gap-1 ${
...
function FilterChip({
  ...
  return (
    <button
      onClick={onClick}
      className={`px-2.5 py-1 text-xs rounded-pill border transition-colors whitespace-nowrap ${
        active
          ? 'bg-sage-600 text-paper border-sage-600'
          : 'bg-paper text-ink-600 border-ink-200 hover:border-sage-300 hover:bg-sage-50'
      }`}
    >
      {label}
    </button>
