...
export default function RelatedIdeasLinker({ ideaId, relatedIds, onChange }: RelatedIdeasLinkerProps) {
  ...
  // Related ideas resolved to their titles
  const relatedIdeas = relatedIds
    .map((id) => allIdeas.find((i) => i.id === id))
    .filter(Boolean) as Idea[];

  // Search results — exclude self and already-linked
      ...
      {relatedIdeas.length > 0 && (
        <ul className="space-y-1 mb-2">
          {relatedIdeas.map((idea) => (
            <li key={idea.id} className="flex items-center gap-2 text-sm group">
              <Link2 className="w-3.5 h-3.5 text-ink-300 shrink-0" />
              <span className="text-ink-700 truncate">{idea.title || 'Untitled'}</span>
              <button
                type="button"
      ...
      {searching ? (
        <div className="relative">
          <input
            ...
            className="w-full px-3 py-1.5 text-sm bg-paper-warm border border-ink-200 rounded-badge outline-none focus:ring-2 focus:ring-sage-400"
            onKeyDown={(e) => {
              if (e.key === 'Escape') { setSearching(false); setQuery(''); }
            }}
          />
          {results.length > 0 && (
            <ul className="absolute left-0 right-0 top-full mt-1 z-20 bg-paper border border-ink-200 rounded-card shadow-modal max-h-48 overflow-y-auto">
              {results.map((idea) => (
                <li key={idea.id}>
                  <button
                    type="button"
                    onClick={() => addRelated(idea.id)}
                    className="w-full text-left px-3 py-2 text-sm text-ink-700 hover:bg-sage-50 transition-colors truncate"
                  >
                    {idea.title || 'Untitled'}
                  </button>
                </li>
          ...
          {query.trim() && results.length === 0 && (
            <div className="absolute left-0 right-0 top-full mt-1 z-20 bg-paper border border-ink-200 rounded-card shadow-modal px-3 py-2 text-xs text-ink-400 italic">
              No matching ideas
            </div>
          )}
          {/* Backdrop to close */}
