...
- **Anchored field generation** — each field suggestion names the upstream field it must use as source of truth (e.g. *The Case* anchors on *Concept*), and a shared anti-hallucination base prompt blocks the model from inventing audiences, markets, deadlines, or technologies that aren't in your notes.
...
**Settings → AI & Agents** (`/settings/ai-agents`) shows one card per service family: Claude, Codex/OpenAI, Local Models, and External / Cloud. Subscription / account-login is the default presentation for Claude and Codex; the API-key path is opt-in from each card's kebab (`⋯`) menu under *Use API key instead*. Local Models picks from Ollama, LM Studio, vLLM, llama.cpp, LocalAI, or a custom localhost URL. External / Cloud covers OpenRouter, Groq, Mistral, Together, Fireworks, or any HTTPS OpenAI-compatible endpoint. Additional named instances ("LM Studio laptop", "OpenRouter personal") accumulate behind a small `+ Add another …` link on each section.

Each saved provider method is a routable provider instance with its own label, URL, model list, enabled-model subset, probe status, and routing identity. When a provider connects, Seedbank discovers its available models and stores them server-side for Feature Defaults and the Ask AI provider/model picker.

**Claude account** uses Seedbank's native OAuth flow with Claude Code–compatible scopes and request shaping (adaptive thinking, context-management edits, current Sonnet/Opus model IDs). **Codex account** uses the local Codex CLI app-server auth flow. If either account was previously authenticated in this browser and later requires sign-in again, Seedbank shows a persistent reauth notice that deep-links into Settings → AI & Agents.

**Provider API keys** (OpenAI, Anthropic, OpenRouter, Groq, Mistral, Together, Fireworks, or any compatible endpoint) are stored server-side, encrypted at rest. Public config responses only expose whether a key exists — the raw value never reaches the browser. These are separate from **Seedbank personal access tokens** (Settings → API & Server), which are bearer tokens for the Seedbank REST API itself.

AI features include Thinking Partner chat, contextual field suggestions, Project drafting, What If, Devil's Advocate, Scope Down, User Story, Idea Health Check, Smart Cross-Pollinate, and Pattern Insights. Field-suggestion prompts are anchored to the relevant upstream field (e.g. *The Case* must use *Concept* as its source of truth), and a shared anti-hallucination base prompt blocks invented audiences, markets, or technologies. All AI features are opt-in. Feature Defaults choose the provider/model/effort for each feature, and the Ask AI modal can temporarily switch to a different configured provider/model for a single run. See [docs/AI_GUIDE.md](docs/AI_GUIDE.md).
